 ...
 * sodass die Liste live neu geleiftet wird — ohne Ghost-Overlay.
 *
 * Die Funktion ist bewusst rein (keine Compose-/Android-Abhängigkeiten),
 ...
 *
 * Sobald [accumPx] eine Zeilenhöhe [heightPx] überschreitet (positiv = nach
 * unten, negativ = nach oben), tauscht die gezogene Zeile mit ihrem direkten
 * Nachbarn über [onSwap] und der Index wandert mit; der Restweg bleibt für
 * den nächsten Schritt erhalten. Auf diese Weise schiebt die gezogene Zeile
 * die übrigen "zur Seite", während der Finger noch gehalten wird.
 *
...
fun reorderStep(
    ...
    // Tauschen, sobald der Finger die Hälfte einer Zeilenhöhe überschreitet.
    // bei vollem height wäre die Schwelle zu hoch für kurze, natürliche Gesten.
    val h = heightPx.coerceAtLeast(1)
    val threshold = h / 2f
    var accum = session.accumPx + dragAmountPx
    // Arbeitskopie: während der Geste wandert die gezogene Zeile hindurch.
